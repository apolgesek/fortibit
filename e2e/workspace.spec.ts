...
test.describe('Workspace > Entry & group', async () => {
  ...
  test('Check entries table - no entries', async () => {
    const noEntriesText = firstWindow.getByText(/there are no entries in this group/i);
  ...
  test('Check entry added', async () => {
    ...
    const noEntriesText = await firstWindow.getByText(/there are no entries in this group/i).count();
  ...
  test('Check entry edited', async () => {
    ...
    expect(await firstWindow.getByPlaceholder(/username/i).inputValue()).toBe('Username1');
  });
  ...
  test('Check entry removed', async () => {
    ...
    const removeBtn = firstWindow.getByRole('dialog').getByRole('button', { name: /remove/i });
    ...
    await firstWindow.getByRole('listitem').getByText(/recycle bin/i).click();
  ...
  test('Check entry moved', async () => {
    await addEntry(firstWindow, { config: { close: true } });
    await firstWindow.getByText(/username1/i).dragTo(firstWindow.getByText(/banking/i));
  ...
  test('Check entries moved', async () => {
    ...
    await firstWindow.getByText(/username1/i).nth(0).dragTo(firstWindow.getByText(/email/i));
    const entriesCount = await firstWindow.getByText(/username1/i).count();
  ...
  test('Check group deleted', async() => {
    ...
    await firstWindow.getByRole('dialog').getByRole('button', { name: /remove/i }).click();
    await firstWindow.getByRole('dialog').waitFor({ state: 'detached' });
    const groupCountAfter = await firstWindow.getByRole('listitem').count();
  ...
  test('Check add entry button disabled/hidden when built-in group active', async () => {
    const trashGroup = firstWindow.getByRole('listitem').getByText(/recycle bin/i)
    ...
    let toolbarAddNewButtonDisabled = await addEntryBtn.getAttribute('disabled');
  ...
  test('Check entry local search', async () => {
    await addEntry(firstWindow, { config: { close: true } });
    await addEntry(firstWindow, { title: 'Aaaa', username: 'Bbbb', config: { close: true } });
  ...
  test('Check entry global search', async () => {
    ...
    await group.click();
    await addEntry(firstWindow, { config: { close: true } });
    await firstWindow.getByRole('button', { name: /search (all|selected) groups?/i }).click();
  ...
  test('Check entry details', async () => {
    ...
    const entryHistoryButton = await firstWindow.getByRole('button', { name: /entry history/i }).count();
    const favoriteButton = await firstWindow.getByRole('button', { name: /add to favorites/i }).count();
    const openLinkButton = await firstWindow.getByRole('button', { name: /add to favorites/i }).count();
  ...
  test('Check sort by creation date', async () => {
    await addEntry(firstWindow, { config: { close: true }, title: 'Abbbb', username: 'user' });
    await addEntry(firstWindow, { config: { close: true }, title: 'Zeeee', username: 'user2' });
    await addEntry(firstWindow, { config: { close: true } });
    ...
    const firstEntry = await firstWindow.getByRole('main').getByRole('listitem').first();
    expect(await firstEntry.innerText()).toMatch(/Abbbb/);

    await firstWindow.getByText(/creation date/i).nth(1).click();
  ...
  test('Check password generated on modal open', async () => {
    ...
    await firstWindow.getByRole('dialog').getByText(/generator/i).waitFor({ state: 'visible' });
  ...
  test('Check password generated when settings changed', async () => {
    ...
    await firstWindow.getByText(/generator/i).click();
    await firstWindow.getByRole('dialog').getByText(/generator/i).waitFor({ state: 'visible' });
    const sliderTrack = firstWindow.getByRole('dialog').getByTestId('slider-track');
    await sliderTrack.hover({ force: true, position: { x: 50, y: 0 } });
    await firstWindow.mouse.down();
    await firstWindow.mouse.up();
    await firstWindow.waitForTimeout(250);
    await firstWindow.getByRole('dialog').getByText(/uppercase letters/i).click();
    const password = await firstWindow.getByTestId('password').innerText();
  ...
  test('Check maintenance scan should remove entry according to input', async () => {
    ...
    await firstWindow.getByRole('menubar').getByText(/maintenance/i).click();
    await firstWindow.getByRole('button', { name: /delete/i }).click();
    
    await expect(firstWindow.getByRole('dialog').getByText(/no entry to delete found/i)).toBeVisible();

    const daysInput = firstWindow.getByRole('dialog').getByLabel(/delete history entries older than/i);
    await daysInput.clear();
    await daysInput.type('0');
    await firstWindow.getByRole('button', { name: /delete/i }).click();

    await expect(firstWindow.getByRole('dialog').getByText(/1\s+entry\s+deleted/i)).toBeVisible();
  });
});
...
test.describe('Workspace > Entry history', async () => {
  async function addHistoryEntry() {
    ...
    await firstWindow.getByRole('dialog').getByRole('button', { name: /confirm/i }).click();
    await firstWindow
      .getByText(/edit entry in general/i)
      .waitFor({ state: 'hidden', timeout: 4000 });
    await firstWindow.getByLabel(/entry history/i).click();
    await firstWindow
      .getByRole('dialog').getByText(/entry history/i)
      .waitFor({ state: 'visible', timeout: 4000 });
  }
  ...
  test('Check history entry added', async () => {
    ...
    const entriesCount = await firstWindow.getByRole('dialog').getByText(/#\d/i).count();
    const title = await firstWindow.getByRole('dialog').getByText(/title1/i).count();
    const username = await firstWindow.getByRole('dialog').getByText(/username1/i).count();
  ...
  test('Check history entry restored', async () => {
    ...
    await firstWindow
      ...
      .waitFor({ state: 'visible', timeout: 4000 });
    await firstWindow.getByRole('dialog').nth(1).getByText(/restore/i).click();
  ...
  test('Check history entry removed', async () => {
    ...
    await firstWindow
      .getByText(/entry history/i)
      .waitFor({ state: 'visible', timeout: 4000 });
    await firstWindow.getByRole('dialog').nth(1).getByText(/delete/i).click();
    await firstWindow.getByRole('dialog').nth(1).waitFor({ state: 'detached' });
...
test.describe('Workspace > File', async () => {
  ...
  test('Save as... option click should save same vault in new file', async () => {
    await authenticate(firstWindow);
    await firstWindow.getByRole('menubar').getByText(/file/i).click();
    await firstWindow.getByRole('menubar').getByText(/^save as...$/i).click();
    await firstWindow.waitForTimeout(2000);
    const invoke = await getInvoke(firstWindow);
    await invoke.evaluate((invoke) => invoke('app:sendInput', `test_${new Date().getTime()}.fbit`));
    await invoke.evaluate((invoke) => invoke('app:sendInput', 13));
  ...
  test('Import option click should import KeePass entries', async () => {
    await authenticate(firstWindow);
    await firstWindow.getByRole('menubar').getByText(/file/i).click();
    await firstWindow.getByRole('menubar').getByText(/import/i).click();
    await firstWindow.getByRole('menubar').getByText(/keepass/i).click();
    await firstWindow.waitForTimeout(2000);
    const invoke = await getInvoke(firstWindow);
    await invoke.evaluate((invoke) => invoke('app:sendInput', 'keepass.xml'));
    await invoke.evaluate((invoke) => invoke('app:sendInput', 13));

    expect(await firstWindow.getByRole('dialog').getByRole('heading').innerText()).toMatch(/import database/i);
    await firstWindow.getByRole('dialog').getByRole('button', { name: /confirm/i }).click();

    expect(await firstWindow.getByRole('alert').innerText()).toMatch(/passwords imported/i);
  });
  ...
  test('Export option click should export entries', async () => {
    ...
    await addEntry(firstWindow, { config: { close: true } });
    await firstWindow.getByRole('menubar').getByText(/file/i).click();
    await firstWindow.getByRole('menubar').getByText(/export/i).click();
    await firstWindow.getByRole('menubar').getByText(/csv/i).click();
    await firstWindow.waitForTimeout(2000);
    const invoke = await getInvoke(firstWindow);
    await invoke.evaluate((invoke) => invoke('app:sendInput', `test_${new Date().getTime()}`));
    await invoke.evaluate((invoke) => invoke('app:sendInput', 13));

    expect(await firstWindow.getByRole('alert').innerText()).toMatch(/database exported/i);
