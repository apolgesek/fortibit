...
export class MenuBarComponent implements OnInit, AfterViewInit {
	...
	ngAfterViewInit() {
		...
		this.zone.runOutsideAngular(() => {
			const windowMove$ = new Observable<void>((subscriber) => {
				this.messageBroker.ipcRenderer.on(
					IpcChannel.RecalculateViewport,
					() => {
						subscriber.next();
					},
				);
	...
	openFile(path?: string) {
		this.workspaceService.openFile(path);
	}
	...
	async import(handler: ImportHandler): Promise<void> {
		try {
			const payload = await this.messageBroker.ipcRenderer.invoke(
	...
	async export(): Promise<void> {
		...
		fileReader.onloadend = async () => {
			const exported = await this.messageBroker.ipcRenderer.invoke(
				IpcChannel.Export,
				fileReader.result,
			);
	...
	private fixMenuSize() {
		...
		const logo: HTMLElement =
			this.topbar.nativeElement.querySelector('.brand-logo');
	...
	private openUrl(path: string) {
		this.messageBroker.ipcRenderer.send(
			IpcChannel.OpenUrl,
			AppConfig.urls.repositoryUrl + path,
		);
	}
}
