...
export class FocusableListDirective implements AfterViewInit {
	...
	@Output() private readonly focused: EventEmitter<ItemFocusedEvent> =
	...
	public onListFocus(event: FocusEvent) {
		if (!this.listStateService.lastFocused) {
			this.focused.emit({
				originalEvent: event,
				item: this._selected ?? this.rowEntries.first.item,
			});
		}
	}
		...
		fromEvent(this.elementRef.nativeElement, 'keydown')
			...
			.subscribe((event: KeyboardEvent) => {
				const selectedItemIndex = this.rowEntries
					.toArray()
					.indexOf(
						this.rowEntries.find(
							(x) => x.item === this.listStateService.selected,
						),
					);
