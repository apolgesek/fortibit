...
export class EncryptionEventService implements IEncryptionEventService {
  ...
  public async getWeakPasswords(database: string, key: string): Promise<{ data: string | false; error: string; }> {
    const encryptionEvent = { database, type: MessageEventType.GetWeakPasswords };
    return await this._encryptionEventWrapper.processEventAsync(encryptionEvent, key) as { error: string, data: string };
  }
  ...
  public async decryptDatabase(data: string, password: string, key: string): Promise<{ error: string; decrypted: string }> {
    const encryptionEvent = { data, password, type: MessageEventType.DecryptDatabase };
    return await this._encryptionEventWrapper.processEventAsync(encryptionEvent, key) as { error: string, decrypted: string };
  }
}