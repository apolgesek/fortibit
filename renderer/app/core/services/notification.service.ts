...
export class NotificationService {
	...
	public add(model: Toast) {
		...
		const notification = (
			componentRef.hostView as EmbeddedViewRef<NotificationComponent>
		).rootNodes[0] as HTMLElement;
		const root = (this.appRef.components[0].hostView as EmbeddedViewRef<any>)
			.rootNodes[0] as HTMLElement;
	...
	private isEqual(
		toast: ComponentRef<NotificationComponent>,
		model: Toast,
	): boolean {
		return (
			toast.instance.model.message === model.message &&
			toast.instance.model.type === model.type
		);
	}
}
