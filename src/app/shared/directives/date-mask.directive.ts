...
export class DateMaskDirective implements ControlValueAccessor {
  ...
  public onPaste(event: ClipboardEvent) {
    ...
    const filtered = event.clipboardData.getData('text').split('').filter(x => (/^[0-9]$/).test(x));
  ...
  public onKeyDown(event: KeyboardEvent) {
    ...
    const date = [dateParts[1], dateParts[0], dateParts[2]].join(this.separator);
    const dateFormatted = [dateParts[0], dateParts[1], dateParts[2]].join(this.separator);
